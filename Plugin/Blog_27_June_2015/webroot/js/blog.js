...
function addCustomField()
{
	jQuery('#AddCustomField').click(function(){
			...
			var result			 = '<div class="Newfild xrow"><div class="row"><div class="col-md-12"><div class="col-md-6"><div class="form-group"><div class="input text"><input type="text" placeholder="Name" id="BlogMetaName_'+index+'" class="form-control" name="data[BlogMeta]['+index+'][title]" value="'+BlogMetaName+'"><button  class="btn grey m5 CustomFieldRemoveButton" rel=""   type="button">Delete</button></div></div></div><div class="col-md-6"><div class="form-group"><div class="input textarea"><textarea id="BlogMetaValue_'+index+'" class="form-control" cols="3" rows="2" name="data[BlogMeta]['+index+'][value]">'+BlogMetaValue+'</textarea></div></div></div><div class="clearfix"></div></div></div></div>';
...
{	
	...
	jQuery('.CustomFieldRemoveButton').click(function(){
			...
			if(removeId != "")
			{
					url= BASE_URL+'admin/blog/blogs/ajax_delete/'+removeId;
					$.ajax({
						url: url,
						type: 'POST',
						dataType: 'json',
...
{	
	...
	jQuery('.addNewBlogCategoryBtn').click(function(){
			...
			if(newCategory != "")
			{
					url= BASE_URL+'admin/blog/blog_categories/ajax_add_category/'+newCategory;
					$.ajax({
						url: url,
						type: 'POST',
						//dataType: 'json',
