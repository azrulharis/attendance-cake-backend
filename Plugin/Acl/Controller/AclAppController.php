...
{
    public $components = array('RequestHandler', 'Acl.AclManager', 'Acl.AclReflector');
	...
	private function _check_config()
	{
		...
		if(!empty($role_model_name))
		{
			...
			//pr($role_model_name);
			//pr(Configure :: read('acl.aro.user.model'));
			//pr($this->_get_role_primary_key_name());
			///pr($this->_get_user_primary_key_name()); 
			//pr($this->_get_role_foreign_key_name());
    	    ...
    	    if($this->AclManager->controller_hash_file_is_out_of_sync())
    	    {
        		...
        		if($has_updates)
        		{
        		   // $this->render('/Acos/admin_has_updates');
        		    //$this->response->send();
					$this->Session->setFlash(__('Some modification find in controller.Please click on generate action for better performance.'),'notification');
