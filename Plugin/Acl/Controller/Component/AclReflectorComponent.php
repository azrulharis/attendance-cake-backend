...
class AclReflectorComponent extends Component
{
	...
	/*
	Abstract : Access All Plugin Comtroller Array
	Created By: Rahul Dev Sharma
	Created On : 29 March 2015 :: 1:16 AM
	*/
	public function plugin_controllers_array($filter_default_controller = false)
	{
		$plugin_paths = $this->get_all_plugins_paths();
			...
			if(!empty($didCD))
			{
				...
				foreach($files as $fileName)
				{
					...
					if(!$filter_default_controller || Inflector::camelize($plugin_name) . 'Controller' != $controller_class_name)
					{
    					...
    					if (!preg_match('/^'. Inflector::camelize($plugin_name) . 'App/', $controller_class_name))
    					{
						    $plugins_controllers[Inflector::camelize($plugin_name)][] = substr($controller_class_name, 0, strlen($controller_class_name) - strlen('Controller'));
    					}
					}
				}
			}
	...
	/*
	Abstract : Access All App Controller Array
	Created By: Rahul Dev Sharma
	Created On : 29 March 2015 :: 1:16 AM
	*/
	public function app_controllers_array()
	{
		$controllers = array();
		...
		if(!empty($didCD))
		{
		    ...
		    foreach($files as $fileName)
			{
				...
				if($controller_class_name != 'AppController')
				{
					$controllers[] = substr($controller_class_name, 0, strlen($controller_class_name) - strlen('Controller'));
				}
			}
		}
	...
	/*
	Abstract : Access All Comtroller Array
	Created By: Rahul Dev Sharma
	Created On : 29 March 2015 :: 1:16 AM
	*/
	public function all_controllers_array()
	{
		$all_controller = array();
		$plugin_controllers = $this->plugin_controllers_array();
		$app_controllers = $this->app_controllers_array();
		
		$all_controller = array(
			'Plugin' => $plugin_controllers,
			'App' => $app_controllers
		);
