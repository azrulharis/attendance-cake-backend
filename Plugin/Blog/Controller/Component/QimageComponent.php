<?php
	...
	class QimageComponent extends Component{
		...
		public function resize($data){
			...
			$source = ($mime == 'image/png') ? imagecreatefrompng($data['file']) : imagecreatefromstring(file_get_blogs($data['file']));
		...
		private function _verifyMime($file){
			...
			$mimes = array('jpeg', 'jpg', 'png', 'gif','ico');
		...
		function resize_crop($file, $destination, $w, $h) {
